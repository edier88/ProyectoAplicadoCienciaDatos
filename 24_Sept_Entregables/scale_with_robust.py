...
def escalar_datasets(input_train="train-70/",
    ...
    for train_file in train_files:
        ...
        # adicionado ---
        #df_train['PORCENTAJE.USO'] = pd.to_numeric(df_train['PORCENTAJE.USO'], errors='coerce')
        #df_test['PORCENTAJE.USO'] = pd.to_numeric(df_test['PORCENTAJE.USO'], errors='coerce')
        # Usar la función
        #df_train = convertir_a_float_seguro(df_train, 'PORCENTAJE.USO')
        #df_test = convertir_a_float_seguro(df_test, 'PORCENTAJE.USO')
        """"
        df_train['PORCENTAJE.USO'] = pd.to_numeric(
            df_train['PORCENTAJE.USO'].astype(str).str.replace(',', '.'), 
            errors='coerce'
        )

        df_test['PORCENTAJE.USO'] = pd.to_numeric(
            df_test['PORCENTAJE.USO'].astype(str).str.replace(',', '.'), 
            errors='coerce'
        )
        """
        # adicionado ---
        ...
        print(f"Tipo de dato PORCENTAJE.USO en train: {df_train['PORCENTAJE.USO'].dtype}")
        print(f"Tipo de dato PORCENTAJE.USO en test: {df_test['PORCENTAJE.USO'].dtype}")

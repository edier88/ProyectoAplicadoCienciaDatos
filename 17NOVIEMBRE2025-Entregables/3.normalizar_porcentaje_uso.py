...
def normalizar_porcentaje(valor):
    ...
    - Los valores sin %:
      ...
      * Si el valor es > 1, se asume que ya es porcentaje y solo se agrega el % (ej: '261273,00' -> '261273,00%')
    """
    ...
    try:
        ...
        if valor_float > 1:
            # Ya es porcentaje, solo agregar el símbolo %
            # Mantener el formato original con coma
            valor_final = f"{valor_str}%"
        else:

...
GRAF_DIR = "Perceptron_Graficas_TrainTest"
...
for archivo in archivos:
    ...
    # Prediccion futura
    forecaster_ajustado_futuro = ForecasterRecursive(
        regressor=MLPRegressor(
            hidden_layer_sizes=hidden_layer_ajustado,
            activation=activation_ajustado,
            solver=solver_ajustado,
    ...
    forecaster_ajustado_futuro.fit(
        y=df['USAGE_KB_scaled'],  # Scaled target
        exog=df[exog_variables_scaled]    # Includes scaled exogenous variables
    )

    predictions_scaled_ajustado_futuro = forecaster_ajustado_futuro.predict(
        steps=7,
        exog=exog_7_dias[exog_variables_scaled]  # Future scaled exogenous variables
    )

    # Desescalado de la prediccion con los hiperparámetros encontrados en la grilla:
    predictions_final_ajustado_futuro = pd.Series(
        scaler_usage.inverse_transform(predictions_scaled_ajustado_futuro.values.reshape(-1, 1)).flatten(),
        index=predictions_scaled_ajustado_futuro.index
    )
    ...
    plt.plot(predictions_final_ajustado_futuro, label="Tráfico Predicho", linewidth=2)
    plt.title(f"MLP - {nombre_zona}")
    plt.xlabel("Índice temporal")
    plt.ylabel("Tráfico (kB)")
    plt.legend()

...
zona_seleccionada_pred = st.sidebar.selectbox(
    "Seleccionar Zona WiFi:",
    options=zonas_disponibles,
    key="zona_prediccion"
)

modelo_prediccion = st.sidebar.selectbox(
    "Seleccionar Modelo:",
    options=["Random Forest", "SVR", "Mejor Modelo por Zona"],
    key="modelo_prediccion"
)

st.sidebar.markdown("---")
st.sidebar.subheader("📝 Parámetros del Día a Predecir")
...
porcentaje_uso = st.sidebar.slider(
    "Porcentaje de Uso (%):",
    min_value=0.0,
    max_value=100.0,
    value=50.0,
...
tipo_dia = st.sidebar.selectbox(
    "Tipo de Día:",
    options=["LABORAL", "FIN_DE_SEMANA", "FESTIVO"],
    key="tipo_dia"
)

es_festivo = st.sidebar.selectbox(
    "¿Es Día Festivo?",
    options=["NO", "SÍ"],
    key="es_festivo"
)

laboral = st.sidebar.selectbox(
    "¿Es Día Laboral?",
    options=["SÍ", "NO"],
    key="laboral"
)

fin_semana = st.sidebar.selectbox(
    "¿Es Fin de Semana?",
    options=["NO", "SÍ"],
    key="fin_semana"
)
...
# ============================================================================
# SECCIÓN E: PREDICCIÓN INTERACTIVA (ÁREA PRINCIPAL)
# ============================================================================
# Mostrar resultados solo si se presionó el botón
if boton_prediccion:
    st.markdown("---")
    st.header("🔮 Predicción de Tráfico WiFi")
    ...
    with st.spinner("Procesando predicción..."):
        try:
            ...
            # Crear tabla de resultados simulada (por ahora)
            # En producción, aquí se haría la predicción real
            resultados_prediccion = pd.DataFrame({
                'Fecha': [fecha_prediccion],
                'Zona': [zona_seleccionada_pred],
                'Modelo': [modelo_prediccion],
                'Tráfico_Predicho_KB': [numero_conexiones * porcentaje_uso / 100 * 10],  # Simulación
            ...
            if not zona_metricas.empty:
                ...
                with col_met3:
                    st.metric("Tráfico Predicho (KB)", f"{resultados_prediccion.iloc[0]['Tráfico_Predicho_KB']:.2f}")
                
                with col_met4:
                    st.metric("Zona", zona_seleccionada_pred)
            
            # Gráfico de tendencia (simulado)
            st.subheader("📉 Tendencia de Tráfico con Predicción")
            
            # Crear datos para el gráfico (simulación de últimos días + predicción)
            fechas_tendencia = pd.date_range(end=fecha_prediccion, periods=30, freq='D')
            trafico_simulado = np.random.normal(numero_conexiones * porcentaje_uso / 100 * 10, 
                                                numero_conexiones * porcentaje_uso / 100 * 2, 30)
            trafico_simulado[-1] = resultados_prediccion.iloc[0]['Tráfico_Predicho_KB']
            
            df_tendencia = pd.DataFrame({
                'Fecha': fechas_tendencia,
            ...
            fig_tendencia = px.line(
                ...
                x='Fecha',
                y='Tráfico_KB',
                title=f'Tendencia de Tráfico - {zona_seleccionada_pred}',
            ...
            fig_tendencia.add_scatter(
                x=[fecha_prediccion],
                y=[resultados_prediccion.iloc[0]['Tráfico_Predicho_KB']],
                mode='markers',
            ...
            with st.expander("ℹ️ Información sobre la Predicción"):
                ...
                **Modelo Utilizado:** {modelo_prediccion}
                ...
                **Nota:** Esta es una predicción simulada basada en los parámetros ingresados.
                Para predicciones completas, el sistema requiere los últimos 7 días de datos históricos
                y los modelos entrenados correspondientes.
                """)
    ...
    # Mostrar información cuando no se ha hecho predicción
    st.markdown("---")
    st.header("🔮 Predicción Interactiva de Tráfico WiFi")
    st.info("👈 **Utilice el panel lateral izquierdo para seleccionar la zona WiFi y los parámetros del día a predecir, luego presione 'Generar Predicción' para ver los resultados.**")

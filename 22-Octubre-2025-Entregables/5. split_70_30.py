...
- Reads all .csv files from `csv-zonas-wifi-agrupados-sumados` (each one should be 1 row per day).
- Sorts by FECHA.CONEXION (ascending).
- Splits chronologically: first 70% -> train, remaining 30% -> test.
- Writes outputs to `train-70/` and `test-30/` using the same original filename
  (no suffixes added).
...
or with custom paths:
    python split_70_30.py --input csv-zonas-wifi-agrupados-sumados --out-train train-70 --out-test test-30
...
def parse_args():
    ...
    ap.add_argument("--input", "-i", default="csv-zonas-wifi-agrupados-sumados", help="Input folder with normalized CSVs (default: resultados_agrupados)")
...
def process_file(file_path, out_train, out_test, date_col, aggregate_on_duplicates=False):
    ...
    out_train_path = os.path.join(out_train, f"{base_name}.csv")
    out_test_path  = os.path.join(out_test,  f"{base_name}.csv")
